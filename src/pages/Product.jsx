...
function Company() {
      ...
      .then(async (res) => {
        if (res.isConfirmed) {
          if (selectedTrashItems.length === 0) {
            await axios
              .put(
                config.apiPath + "/product/restore/" + item.id,
                config.headers()
              )
              .then((res) => {
            ...
            for (const itemId of selectedTrashItems) {
              const res = await axios.put(
                config.apiPath + "/product/restore/" + itemId,
                config.headers()
              );
              if (res.data.message === "success") {
                Swal.fire({
  ...
  const fetchDataTrash = async () => {
    try {
      const res = await axios.get(
        config.apiPath + "/product/listTrash",
        config.headers()
      );
  ...
  const handleRemoveReal = async (item) => {
    try {
      ...
      if (button.isConfirmed) {
        if (selectedTrashItems.length === 0) {
          const res = await axios.delete(
            config.apiPath + "/product/removeReal/" + item.id,
            config.headers()
          );
          ...
          for (const itemId of selectedTrashItems) {
            const res = await axios.delete(
              config.apiPath + "/product/removeReal/" + itemId,
              config.headers()
            );
  ...
  return (
    <BackOffice>
      ...
      <button
        className="btn btn-outline-warning ml-2 float-center"
        data-toggle="modal"
                ...
                <tr key={item.id}>
                  ...
                  <td className="text-center">
                    <button
                      className="btn btn-danger"
                      disabled={selectedTrashItems.length === 0}
                      onClick={(e) => handleRemoveReal(item)}
                    >
                      <i className="fa fa-times"></i>
                    </button>
                  </td>
